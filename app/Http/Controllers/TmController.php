...
class TmController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        $departemenId = session('departemen_id');
        ...
        if ($periodeId) {
            $topTransportasi->where('spd.periode_id', $periodeId);
        }
        ...
        $dataAnggaranPerDeptPerPeriode = DB::table('anggaran_fix as a')
            ...
            ->when($departemenId, function ($query) use ($departemenId) {
                return $query->where('d.nama', $departemenId);
            })
            ->get();
    ...
    {
        $request->validate([
            'periode_id' => 'nullable|exists:periode_anggaran,id',
        ]);

        session(['periode_id' => $request->periode_id]);
        return redirect()->route('dashboard.tmhcm');
    }
    ...
    public function setDepartment(Request $request)
    {
        ...
        session(['departemen_id' => $request->departemen_id]);
        return redirect()->route('dashboard.tmhcm');
    }
}
